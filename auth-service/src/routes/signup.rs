...
use crate::{app_state::AppState, domain::{data_store::UserStore, email::Email, error::AuthAPIError, password::Password, user::User}, services::{hashmap_user_store::HashmapUserStore, hashset_banned_token_store::HashsetBannedTokenStore}};
// Order of parameters is important in the handler
pub async fn signup(State(state): State<AppState<HashmapUserStore, HashsetBannedTokenStore>>,Json(request): Json<SignupRequest> ) -> Result<impl IntoResponse, AuthAPIError> {
