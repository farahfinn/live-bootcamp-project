...
use crate::domain::{data_store::{LoginAttemptId, TwoFACode, TwoFACodeStore, TwoFACodeStoreError}, email::Email};
...
impl TwoFACodeStore for RedisTwoFACodeStore {
    async fn add_code(
        ...
        let twofatuple = TwoFATuple(login_attempt_id.as_ref().into(), code.as_ref().into());
        ...
        let serialized_2fa = serde_json::to_string(&twofatuple).
        ...
        connection.set_ex(key, serialized_2fa, TEN_MINUTES_IN_SECONDS)
            .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
        Ok(()) 
    }
    ...
    async fn get_code(
        ...
        let mut connection = self.conn.write().await;
        let get_result: Result<String, redis::RedisError> = connection.get(key);
        ...
        match get_result {
            Ok(json_str) => {
                ...
                let two_fa_tuple: TwoFATuple  = serde_json::from_str(&json_str)
                    .map_err(|_| TwoFACodeStoreError::UnexpectedError)?;// Return TwoFACodeStoreError::UnexpectedError if parsing fails.
